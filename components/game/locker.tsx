...
export function Locker({profile,onChange}:{profile:Profile;onChange:(profile:Profile)=>void}) {
 ...
 const skins=CATALOG.filter(item=>'weapon' in item&&item.weapon===weapon);
 ...
 return <div className="arsenal">
  ...
  <div className="arsenal-finish"><div><small><Paintbrush size={13}/> FINISH</small><h3>{item.name.split(' / ').at(-1)}</h3></div><div className="finish-swatches" aria-label="Choose finish">{finishes.map(f=><button key={f.id} title={f.name.split(' / ').at(-1)} aria-label={f.name.split(' / ').at(-1)} aria-pressed={item.id===f.id} onClick={()=>setSelection(f.id)} style={{'--swatch':f.color} as React.CSSProperties}>{f.id!=='finish-factory'&&!profile.owned.includes(f.id)?<Lock size={12}/>:weaponFinish(profile,weapon)===f.variant?<Check size={14}/>:null}</button>)}</div>
   <button className="arsenal-equip" disabled={equipped||(!owned&&profile.balance<item.cost)} onClick={()=>onChange(owned?equipWeaponFinish(profile,weapon,item.id):purchase(profile,item.id))}>{equipped?<><Check size={15}/> EQUIPPED</>:owned?'EQUIP':item.cost===0?'UNLOCK FREE':profile.balance<item.cost?`${item.cost-profile.balance} KM NEEDED`:`BUY · ${item.cost} KM`}</button>
  </div>
 </div>;
}
