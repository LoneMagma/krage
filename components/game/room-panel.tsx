...
export function RoomPanel({mode,map,primary,operator=0,name,onName,ready,info,onConnect,onChange,onStart,onLeave}: {
  ...
  return <div className={'room-panel '+(lobby?'party-room':'room-builder')}>
    {lobby ? <>
      ...
      <div className="section-tabs"><button aria-pressed={screen==='create'} onClick={()=>setScreen('create')}>CREATE LOBBY</button><button aria-pressed={screen==='join'} onClick={()=>setScreen('join')}>JOIN LOBBY</button></div>
