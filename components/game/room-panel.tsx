...
export function RoomPanel({mode,map,primary,operator=0,name,onName,ready,info,onConnect,onChange,onStart,onLeave}: {
  ...
  const [section,setSection]=useState('ARENA');
  ...
  const loadout=<div className="room-options">
    ...
    {select('CHARACTER',own?.operator??character,n=>{setCharacter(n);if(lobby)onChange({operator:n});},['MALE','FEMALE'],(lobby?.mode??chosenMode)>=2)}
  </div>;
  return <div className={'room-panel '+(lobby?'party-room':'room-builder')}>
    {lobby ? <>
      ...
      <div className="party-layout"><div className="party-lineup"><PartyPreview players={lobby.actors}/></div><aside className="party-rules"><h3>MATCH</h3><div className="rule-summary"><span>{['FFA','1 v 1','2 v 2','3 v 3'][lobby.mode]}</span><span>{lobby.map===0?'DUNE':'SNOW'}</span><span>{(lobby.duration??300)/60} MIN</span><span>{lobby.fragLimit} FRAGS</span><span>{lobby.capacity} SLOTS</span><span>{lobby.botFill?`${lobby.difficulty?.toUpperCase()} BOTS`:'NO BOTS'}</span></div><h3>YOUR LOADOUT</h3>{loadout}</aside></div>
      ...
      <div className="section-tabs"><button aria-pressed={screen==='create'} onClick={()=>setScreen('create')}>CREATE LOBBY</button><button aria-pressed={screen==='join'} onClick={()=>setScreen('join')}>JOIN FRIENDS</button></div>
      <label>PLAYER NAME<input aria-label="Player name" value={name} maxLength={16} onChange={e=>onName(e.target.value)}/></label>
      {screen==='join'?<form className="room-join" onSubmit={e=>{e.preventDefault();connect(true);}}><label>INVITE CODE<input aria-label="Invite code" maxLength={6} value={room} placeholder="ABC123" onChange={e=>setRoom(e.target.value.toUpperCase())}/></label><Button type="submit" disabled={!ready||busy||!/^[A-F0-9]{6}$/.test(room)}>JOIN</Button></form>:<>
        <div className="room-config"><nav aria-label="Custom game sections">{['ARENA','RULES','BOTS','LOADOUT'].map(label=><button key={label} aria-pressed={section===label} onClick={()=>setSection(label)}>{label}</button>)}</nav><div className="room-config-fields">
          <h3>{section}</h3>
          {section==='ARENA'&&<div className="room-options">{select('MODE',chosenMode,setMode,['FFA','1 v 1','2 v 2','3 v 3'])}{select('MAP',chosenMap,setMap,['DUNE','SNOW'])}</div>}
        ...
        <div className="room-recap"><span>{chosenMap===0?'DUNE':'SNOW'} · {['FFA','1 v 1','2 v 2','3 v 3'][chosenMode]}</span><span>{duration/60} MIN · {fragLimit} FRAGS</span></div>
