...
export function RoomPanel({mode,map,primary,operator=0,weaponFinishes=[],name,onName,ready,info,onConnect,onChange,onStart,onLeave,onKick}:{
 ...
 const lobby=info?.lobby,own=lobby?.actors.find(a=>a.id===lobby.you),busy=info?.status==='connecting'||info?.status==='reconnecting';
 ...
 return <div className="room-panel party-first">
  ...
  {lobby?<>
   ...
   {copyFailed&&<input aria-label="Invite link" readOnly value={`${location.origin}/?room=${lobby.room}`} onFocus={e=>e.target.select()}/>}
   ...
   {!settings&&<PartyPreview players={[{id:0,name:name||'PLAYER',team:0,operator:chosenMode>=2?0:character,primary:chosenPrimary,weaponFinishes,bot:false,connected:true,ready:false}]} capacity={Math.min(4,slots)} you={0}/>}
