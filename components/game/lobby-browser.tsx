...
export function LobbyBrowser({onJoin,onQuick,busy}:{onQuick:()=>void;onJoin:(room:string,password:string)=>void;busy:boolean}){
 ...
 return <div className="lobby-browser"><div className="lobby-browser-title"><strong>ACTIVE LOBBIES</strong><button aria-label="Refresh lobbies" onClick={()=>{setStatus('FINDING LOBBIES');setRefresh(n=>n+1);}}><RefreshCw size={17}/></button></div>
  {status&&<output>{status}</output>}
  <div className="lobby-list"><div className="lobby-list-row default-lobby"><Users size={28}/><div><strong>DEFAULT LOBBY</strong><small>QUICK PLAY</small></div><span className="lobby-occupancy">OPEN</span><button disabled={busy} onClick={onQuick}>JOIN</button></div>{rooms.slice(active*2,active*2+2).map(r=><div className="lobby-list-row" key={r.room}>
