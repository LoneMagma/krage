...
export function LobbySection({open,playing,title,kind,onClose,children}: {open:boolean;playing:boolean;title:string;kind:string;onClose:()=>void;children:ReactNode}) {
  ...
  if(!open)return null;
  return <section className={'lobby-section section-'+kind} aria-label={title}><header><div><h2 tabIndex={-1} ref={heading}>{title}</h2></div><button className="back-to-play" onClick={onClose} aria-label="Return to play">← PLAY</button></header><div className="lobby-section-body" key={kind}>{children}</div></section>;
}
